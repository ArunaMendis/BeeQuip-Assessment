...
export class FindTruckPage {
  constructor(page) {
    ...
    this.inputStartYear = page.locator('[data-hook="aggregation-yearOfConstruction-from"]');
    this.inputEndYear = page.locator('[data-hook="aggregation-yearOfConstruction-to"]');
    this.kilometerStand = page.locator('[id="radix-:R53lql6:"]');
    this.kilometerStandLabel = page.locator('[id="radix-:Rd3lql6:"]');
    this.usageInKilometersTo = page.locator('[data-hook="aggregation-usageInKilometers-to"]');
    this.numberOfCylinders = page.getByText("Aantal cilinders");
    this.NumberOfCylindersCheckbox = page.locator('[data-hook="aggregation-label"]');
    this.selectFirstSearchResultOption = page.locator('[data-hook="object-card"]');
  }

  async setYearOfConstruction(startYear, endYear) {
    await this.yearConstruction.evaluate((el) =>el.scrollIntoView({ behavior: "smooth", block: "center" }));
    ...
    const submitButton = this.yearConstructionLabel.locator('button[type="submit"]');
    await submitButton.click();
  }

  async setUsageInKilometersTo(kilometers) {
    await this.kilometerStand.evaluate((el) =>el.scrollIntoView({ behavior: "smooth", block: "center" }));
    ...
    const submitButton = this.kilometerStandLabel.locator('button[type="submit"]');
    await Promise.all([
...
async setNumberOfCylinders(numberOfCylinders) {
    ...
    const checkbox = this.NumberOfCylindersCheckbox.filter({ hasText: numberOfCylinders });
